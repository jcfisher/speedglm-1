...
speedglm <- function(formula,data,family=gaussian(),weights=NULL,start=NULL,
                     ...
                     sparse=NULL,trace=FALSE,set.default=list(), 
                     method=c('qr','eigen','Cholesky'), model=FALSE, target=FALSE, 
                     fitted=FALSE,...){
  call <- match.call()
  tf <- terms(formula)
...
speedglm.wfit <- function(y, X, intercept=TRUE, weights=NULL,row.chunk=NULL,
                          ...
                          tol.vectors=1e-7, tol.solve=.Machine$double.eps,trace=FALSE,
                          method = c('qr','eigen','Cholesky'), sparse=NULL,...){
  nobs <- NROW(y)
  nvar <- ncol(X) 
...
shglm <- function(formula, datafun, family = gaussian(), weights.fo = NULL,
  ...
  rval <- list(coefficients = coefficients, logLik = ll.nuovo,
               iter = iter, tol = tol, family = fam, link = link, df = resdf,
               ...
               aic = aic.model, convergence = (!(tol> set$acc)))
  rval$tf <- tf
  rval$call <- call
  if ((rval$iter==maxit)&(!rval$convergence))
    warning("Maximum number of iterations reached without convergence")

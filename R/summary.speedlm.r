summary.speedlm <- function (object, correlation = FALSE, ...) 
{
  ...
  z <- object
  n <- if (is.null(z$weights)) 
    z$n.obs
  else z$n.obs - z$zero.w
  nvar <- z$nvar
  rdf <- z$df.residual
  if(z$method=='qr') {
  ...
  if (is.null(z$weights)) {
    ...
    mss <- if (z$intercept) {
      ...
      XW1 <- matrix(kronecker(XW1 * z$SW, XW1), z$rank, 
                    z$rank, byrow = TRUE)
      crossprod(z$coef, z$XTX - 2 * XWX/z$SW + XW1/(z$SW^2)) %*% 
        z$coef
    }
...
#nobs.speedlm <- function(object, ...) return(object$n)

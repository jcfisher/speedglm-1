speedlm.fit <- function (y, X, intercept = FALSE, offset = NULL, row.chunk = NULL, 
                         ...
                         sparse = NULL, tol.values = 1e-07, tol.vectors = 1e-07,
                         method=c('qr','eigen','Cholesky'), ...) 
{
  ...
  rval <- list(coefficients = coefficients, coef = coef, df.residual = dfr, 
               XTX = as(ris$XTX, "matrix"), Xy = Xy, n.obs = nrow(X), 
                        ...
                        method=c('qr','eigen','Cholesky'), ...) 
{
  ...
  rval <- list(coefficients = coefficients, coef = coef, weights = w, 
               ...
               n.obs = nrow(X), nvar = nvar, ok = ok, A = as(A, "matrix"), 
...
speedlm <- function(formula, data, weights = NULL, offset = NULL, sparse = NULL, 
                    set.default = list(), model = FALSE, method=c('qr','eigen','Cholesky'), 
                    target = FALSE, fitted = FALSE, ...) 
{
  call <- match.call()
  tf <- terms(formula)
...
updateWithMoreData <- function(object, data, weights = NULL, offset = NULL,
  ...
  if ((!(all.levels))&(length(fa)>0)) {
    ...
    for (i in 1:length(fa)) {
      ...
      eval(parse(text = paste("flevels$'", names(M)[fa[i]],
                              "'", "<-levels(M[,fa[i]])", sep = "")))
      a <- c(object$levels[[j]][!(object$levels[[j]] %in%
                                    flevels[[j]])], flevels[[j]])
  ...
  rval <- list(coefficients = coefficients, coef = coef, df.residual = dfr, 
               ...
               A = as(A,"matrix"), n.obs = object$n.obs + nrow(X), RSS = as.numeric(RSS), 
